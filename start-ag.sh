...
if command -v docker >/dev/null 2>&1; then
  if [ -f "$COMPOSE_FILE" ]; then
    ...
    docker compose -f "$COMPOSE_FILE" up -d neo4j redis tempo loki prometheus grafana otel-collector >/tmp/ag_full_stack.log 2>&1 || echo "[start-ag] Warning: failed to start full stack (see /tmp/ag_full_stack.log)"
  else
