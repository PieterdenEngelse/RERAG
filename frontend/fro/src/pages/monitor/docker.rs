...
pub fn MonitorDocker() -> Element {
    ...
    let mut show_cs_info = use_signal(|| false);
        ...
        div { class: "space-y-6",
            ...
            if snapshot.loading {
                ...
                if show_cs_info() {
                    div {
                        class: "fixed inset-0 z-50 flex items-center justify-center bg-black/60",
                        onclick: move |_| show_cs_info.set(false),
                        div {
                            class: "bg-gray-800 border border-gray-600 rounded-lg p-5 w-[90vw] max-w-lg max-h-[90vh] overflow-y-auto shadow-xl",
                            onclick: move |evt| evt.stop_propagation(),
                            div { class: "flex items-center justify-between mb-3",
                                h2 { class: "text-base font-semibold text-gray-100", "Container Status" }
                                button {
                                    class: "text-gray-400 hover:text-gray-200 text-xl font-bold",
                                    onclick: move |_| show_cs_info.set(false),
                                    "\u{d7}"
                            ...
                            div { class: "text-sm text-gray-300 space-y-2",
                                p { "Each card represents a Docker container in the ag observability stack." }
                                p { "Most containers only need the Docker health badge. However, " strong { "Redis" } " is a special case:" }
                                ul { class: "list-disc ml-5 space-y-1",
                                    li { "The Docker badge shows whether the Redis container is running." }
                                    li { "The \"backend connected\" badge shows whether the ag backend has an active Redis connection." }
                ...
                Panel {
                    ...
                    div { class: "flex items-end gap-1 mb-3",
                        h3 { class: "text-sm font-semibold text-gray-200", "Container Status" }
                        button {
                            class: PARAM_ICON_BUTTON_CLASS,
                            style: PARAM_ICON_BUTTON_STYLE,
                            title: "Info",
                            onclick: move |_| show_cs_info.set(true),
                            svg {
...
fn ContainerInfoBody(name: String) -> Element {
    match name.as_str() {
        ...
        "otel" => rsx! {
            p {
                "OpenTelemetry (OTel) is an open standard for observability. It provides distributed tracing — every HTTP request gets a "
                code { "trace_id" }
                " assigned by the trace middleware."
            }
            p { class: "mt-2",
                "The backend sends trace data via OTLP HTTP to the OTel Collector on port 4318, which forwards to Tempo on port 4317. Grafana queries Tempo to display traces. The "
                code { "trace_id" }
                " in every log line links that entry to its full trace in Tempo/Grafana."
            }
            p { class: "mt-2",
                "The backend sends traces in a fire-and-forget manner — no persistent connection is maintained. If the Collector is unreachable, traces are silently dropped and the backend continues normally."
            }
