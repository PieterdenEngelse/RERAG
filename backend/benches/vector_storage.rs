...
fn bench_rkyv_serialize(c: &mut Criterion) {
    ...
    for count in [100, 1000, 5000].iter() {
        ...
        group.bench_with_input(
            ...
            |b, storage| {
                b.iter(|| {
                    black_box(rkyv::to_bytes::<rkyv::rancor::Error>(storage).unwrap())
                });
            },
        );
    }
...
fn bench_rkyv_access(c: &mut Criterion) {
    ...
    for count in [100, 1000, 5000].iter() {
        ...
        group.bench_with_input(
            ...
            |b, bytes| {
                b.iter(|| {
                    let archived = rkyv::access::<ArchivedBenchVectorStorage, rkyv::rancor::Error>(bytes).unwrap();
        ...
        group.bench_with_input(
            ...
            |b, bytes| {
                b.iter(|| {
                    ...
                    let vecs: Vec<Vec<f32>> = archived.vectors
                        .iter()
...
fn bench_size_comparison(c: &mut Criterion) {
    ...
    for count in [100, 1000, 5000].iter() {
        ...
        let rkyv_storage = BenchVectorStorage {
            version: 1,
            vectors: vectors.clone(),
            doc_id_to_idx: doc_map.iter().map(|(k, v)| (k.clone(), *v as u32)).collect(),
        };
