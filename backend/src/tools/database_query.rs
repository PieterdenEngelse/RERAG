...
impl Tool for DatabaseQueryTool {
    ...
    async fn execute(&self, query: &str) -> Result<ToolResult, String> {
        ...
        if !Self::is_safe_query(query_trimmed) {
            warn!("DatabaseQueryTool: blocked unsafe query '{}'", query_trimmed);
            return Ok(ToolResult {
                tool: ToolType::DatabaseQuery,
                success: false,
                result: "Only SELECT queries are allowed. Use 'show tables' to see available tables.".to_string(),
                metadata: ToolMetadata {
                    execution_time_ms: start.elapsed().as_millis() as u64,
                    confidence: 0.0,
                    source: Some("DatabaseQuery".to_string()),
...
mod tests {
    ...
    fn test_safe_query_detection() {
        ...
        assert!(DatabaseQueryTool::is_safe_query("SELECT id, name FROM users WHERE id = 1"));
        assert!(!DatabaseQueryTool::is_safe_query("INSERT INTO users VALUES (1, 'test')"));
        assert!(!DatabaseQueryTool::is_safe_query("DELETE FROM users"));
        assert!(!DatabaseQueryTool::is_safe_query("DROP TABLE users"));
        assert!(!DatabaseQueryTool::is_safe_query("UPDATE users SET name = 'test'"));
        assert!(!DatabaseQueryTool::is_safe_query("SELECT * FROM users; DROP TABLE users"));
    }
