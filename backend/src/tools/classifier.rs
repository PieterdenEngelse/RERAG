...
impl ClassifierTool {
    pub fn new() -> Self {
        ...
        // Document categories
        categories.insert("technical".to_string(), vec![
            "code", "programming", "api", "function", "class", "method", "algorithm",
            "database", "server", "client", "framework", "library", "module", "package",
            "debug", "error", "exception", "compile", "runtime", "syntax",
        ]);
        categories.insert("business".to_string(), vec![
            "revenue", "profit", "sales", "marketing", "customer", "client", "contract",
            "budget", "forecast", "strategy", "growth", "market", "competition",
            "stakeholder", "roi", "kpi", "metrics", "performance",
        ]);
        categories.insert("scientific".to_string(), vec![
            "research", "study", "experiment", "hypothesis", "data", "analysis",
            "methodology", "results", "conclusion", "peer-review", "journal",
            "citation", "abstract", "findings", "statistical",
        ]);
        categories.insert("legal".to_string(), vec![
            "contract", "agreement", "clause", "liability", "compliance", "regulation",
            "law", "statute", "court", "plaintiff", "defendant", "jurisdiction",
            "intellectual property", "patent", "trademark", "copyright",
        ]);
        categories.insert("educational".to_string(), vec![
            "learn", "teach", "course", "lesson", "tutorial", "guide", "example",
            "exercise", "practice", "student", "instructor", "curriculum",
            "assessment", "quiz", "exam", "certification",
        ]);
        categories.insert("support".to_string(), vec![
            "help", "issue", "problem", "error", "fix", "solution", "troubleshoot",
            "ticket", "request", "urgent", "priority", "escalate", "resolve",
        ]);

        let mut intents = HashMap::new();
        
        // User intents
        intents.insert("question".to_string(), vec![
            "what", "how", "why", "when", "where", "who", "which", "can", "could",
            "would", "should", "is", "are", "does", "do", "?",
        ]);
        intents.insert("command".to_string(), vec![
            "create", "delete", "update", "add", "remove", "change", "modify",
            "set", "get", "list", "show", "display", "run", "execute", "start", "stop",
        ]);
        intents.insert("search".to_string(), vec![
            "find", "search", "look", "locate", "discover", "retrieve", "fetch",
            "query", "filter", "browse",
        ]);
        intents.insert("comparison".to_string(), vec![
            "compare", "versus", "vs", "difference", "between", "better", "worse",
            "pros", "cons", "advantages", "disadvantages",
        ]);
        intents.insert("explanation".to_string(), vec![
            "explain", "describe", "elaborate", "clarify", "define", "meaning",
            "understand", "tell me about",
        ]);
    ...
    fn classify_category(&self, text: &str) -> Vec<(String, f32)> {
        ...
        let mut scores: Vec<(String, f32)> = self.categories
            .iter()
    ...
    fn analyze_sentiment(&self, text: &str) -> Sentiment {
        ...
        if text.contains('?') || text_lower.starts_with("what") 
            || text_lower.starts_with("how") || text_lower.starts_with("why")
            || text_lower.starts_with("when") || text_lower.starts_with("where")
            || text_lower.starts_with("who") || text_lower.starts_with("can")
            || text_lower.starts_with("could") || text_lower.starts_with("would")
        {
            return Sentiment::Question;
        }

        let positive_words = ["good", "great", "excellent", "amazing", "wonderful", 
            "fantastic", "love", "like", "best", "perfect", "awesome", "helpful",
            "thanks", "thank", "appreciate", "success", "successful"];
        
        let negative_words = ["bad", "terrible", "awful", "horrible", "hate", "worst",
            "poor", "fail", "failed", "error", "bug", "broken", "issue", "problem",
            "wrong", "incorrect", "disappointed", "frustrating"];
    ...
    fn classify(&self, text: &str) -> Classification {
        ...
        let (category, confidence) = categories.first()
            .map(|(c, s)| (c.clone(), *s))
...
impl Tool for ClassifierTool {
    ...
    async fn execute(&self, query: &str) -> Result<ToolResult, String> {
        ...
        output.push_str(&format!("**Category:** {} ({:.0}% confidence)\n", 
            classification.category, classification.confidence * 100.0));
...
mod tests {
    ...
    async fn test_technical_classification() {
        ...
        let result = tool.execute("How do I fix this runtime error in my Python code?").await;
        assert!(result.is_ok());
    ...
    async fn test_question_intent() {
        let tool = ClassifierTool::new();
        let result = tool.execute("What is the best way to learn machine learning?").await;
        assert!(result.is_ok());
        let res = result.unwrap();
